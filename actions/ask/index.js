...
async function main(params) {
  ...
  try {
    ...
    // Use sanitized input
    let sanitizedQuestion = validation.sanitized;
    
    // Detect doc source prefix and override environment variables
    const docSourceInfo = detectDocSource(sanitizedQuestion);
    if (docSourceInfo.prefix) {
      sanitizedQuestion = docSourceInfo.question;
      // Override doc source for this request
      params.DOCS_BASE_URL = docSourceInfo.url;
      params.DOCS_NAME = docSourceInfo.name;
      logger.info(`Doc source override: ${docSourceInfo.name}`);
    }
